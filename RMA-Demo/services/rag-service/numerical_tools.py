...
class NumericalTools:
    ...
    def get_tool_definitions(cls) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "description": "Check if an amount meets a threshold/limit. Provide the threshold VALUE from the context. Example: check_threshold('60000', 'DRO maximum debt', '30000') where '30000' comes from the manual text.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "amount": {
                        ...
                        "threshold_name": {
                            "type": "string",
                            "description": "Description of the threshold (e.g., 'DRO maximum debt', 'bankruptcy fee')"
                        },
                        "threshold_value": {
                            "type": "string",
                            "description": "The limit/threshold VALUE extracted from the manual context (e.g., '30000' if manual says 'limit is £30,000')"
                        }
                    },
    ...
    @staticmethod
    def check_threshold(amount: Union[str, float], threshold_name: str, threshold_value: Union[str, float] = None) -> Dict[str, Any]:
        """
        Check if an amount meets a threshold/limit.
        
        The threshold value can be provided directly or the tool will indicate it needs to be looked up.
        
        Args:
            amount: Amount to check (can include £, commas)
            threshold_name: Name/description of threshold (e.g., "DRO maximum debt")
            threshold_value: The limit value (optional - if not provided, will request lookup)
        ...
        try:
            ...
            # If no threshold value provided, indicate it needs to be looked up from context
            if threshold_value is None:
                return {
                    ...
                    "needs_lookup": True,
                    "message": f"Please provide the {threshold_name} value from the manual context to complete the comparison"
                }
            
            # Clean and convert threshold
            clean_threshold = str(threshold_value).replace('£', '').replace(',', '')
            threshold_val = float(clean_threshold)
            
            # Perform comparison
            is_within = amount_value <= threshold_val
            difference = amount_value - threshold_val
            
            # Generate advice based on threshold name context
            threshold_lower = threshold_name.lower()
            if any(word in threshold_lower for word in ['max', 'maximum', 'limit', 'exceed', 'no more']):
                # It's an upper limit
                if is_within:
                    if difference == 0:
                        advice = f"✓ Amount is exactly at the {threshold_name} limit"
                    else:
                        advice = f"✓ Amount is £{abs(difference):,.2f} below the {threshold_name} - QUALIFIES"
                else:
                    advice = f"✗ Amount is £{difference:,.2f} OVER the {threshold_name} - DOES NOT QUALIFY"
            elif any(word in threshold_lower for word in ['min', 'minimum', 'at least']):
                # It's a lower limit (inverted logic)
                is_within = amount_value >= threshold_val
                if is_within:
                    advice = f"✓ Amount meets or exceeds the {threshold_name} - QUALIFIES"
                else:
                    advice = f"✗ Amount is below the {threshold_name} - DOES NOT QUALIFY"
            else:
                # Generic comparison
                if is_within:
                    advice = f"✓ Amount is within the {threshold_name} threshold"
                else:
                    advice = f"✗ Amount exceeds the {threshold_name} threshold"
            ...
            return {
                ...
                "formatted_threshold": f"£{threshold_val:,.2f}",
                "is_within_limit": is_within,
                "difference": abs(difference),
                "formatted_difference": f"£{abs(difference):,.2f}",
                "advice": advice,
                "comparison": f"£{amount_value:,.2f} {'<=' if amount_value <= threshold_val else '>'} £{threshold_val:,.2f}"
            }
